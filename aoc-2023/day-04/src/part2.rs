...
impl Card {
    ...
    //     2u32.pow(power - 1)
    // }
    fn get_number_of_copy_card(&self) -> u32 {
        self.winning_numbers
            .intersection(&self.player_numbers)
            .count() as u32
    }
    fn add_copy(&mut self, number_of_copies: u32) {
        self.instance += number_of_copies;
    }
}
...
pub fn process(input: &str) -> String {
    let (_input, mut cards) = parse_cards(input).expect("parse cards");
    // dbg!(_input, &cards);
    let copy_cards = cards.to_vec();
    copy_cards.iter().enumerate().for_each(|(id, card)| {
        let copy_card = card.get_number_of_copy_card();
        let number_of_copies = cards[id].instance;
        for copy in 1..=copy_card {
            if id < cards.len() - 1 {
                cards[id + copy as usize].add_copy(number_of_copies);
            }
        }
    });
    // dbg!(&cards);
